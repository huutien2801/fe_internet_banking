...
const debtService = axios.create({
    baseURL: 'https://ibs-api.herokuapp.com/api/v1/debt',
    // baseURL: 'http://35.223.196.250:8080/api/v1/auth',// url = base url + request url
    timeout: 15000 // request timeout
})

debtService.interceptors.request.use(
    config => {
...
// request interceptor
debtService.interceptors.response.use((response) => {
    return response;
}, (error) => {
