...
<tr v-else>
    ...
    <td class="text-center text-wrap" style="vertical-align: middle">
        ...
        <span class="btn btn-dark" v-if="debtObj.status == 'UNDONE'">
            CHƯA THANH TOÁN
        </span>
        <span class="btn btn-outline-danger" v-if="debtObj.status == 'CANCEL'">
            ĐÃ HỦY
    ...
    <div class="modal fade" :id="'confirmPayModal'+debtObj.remind_id" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" @click="onPayRemind">Thanh toán</button>
                    <button type="button" class="btn btn-outline-secondary" data-dismiss="modal">Đóng</button>
                </div>
            </div>
        </div>
...
export default {
    ...
    mounted: function () {
        ...
        if (this.debtObj.reminder_account_number != parseInt(this.userAccountObj.account_number)) {
        ...
        onCancleRemind: async function () {
            let payload = {
                body: {
                    ...
                    message: this.message != '' ? this.message : `STK ${this.debtObj.reminded_account_number} ${this.debtObj.reminded_full_name} vừa thanh toán nợ ${this.debtObj.debt} cho bạn`
                }
            }
